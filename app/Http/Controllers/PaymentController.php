    ...
    public function verify(Request $request)
    {
        ...
        try {
            ...
            if ($t->type == Transaction::DEPOSIT_TYPE) {
                ...
                $history->save();
                $text = "افزایش اعتبار $user->username به مبلغ $t->amount با موفقیت انجام شده است ";
                ...
                $text = $freelancer ? "$employer->username پروژه ی $project->title را برای $freelancer->username ایجاد کرد."
                    : "$employer->username پروژه ی $project->title را ایجاد کرد.";
                ...
                $text = 'ارتقای عضویت '. $user->username .' به پکیج ' . $package->title . ' با موفقیت انجام شد.';
                ...
                $freelancer = $project->freelancer;
                $text = $freelancer->username . 'برای پروژه ' . $project->title . 'پرداخت امن ' . $t->amount . 'ایجاد کرده است';
                $type = Notification::ADMIN_PROJECT;
                Notification::make(
                    $type,
                    $text,
